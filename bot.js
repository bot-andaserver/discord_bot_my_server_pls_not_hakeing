...
client.on('message', async msg => { 
	...
	if (command === `play`) {
		...
		if (!permissions.has('CONNECT')) {
			...
			return msg.channel.send('ليس لدي صلاحيات للدخول في هاد الرروم');
		}
		if (!permissions.has('SPEAK')) {
			return msg.channel.send('ليس لدي صلاحيات للتكلم في هاد الرروم');
		}
...
client.on('guildCreate', guild => {
  var embed = new Discord.RichEmbed()
  .setColor(0x5500ff)
  .setDescription(`**Thank You For Adding The Bot To Your Server If You Need Any Help In The Bot Go To Suuport Server**`)
...
var fkk =[
        ...
        {n:"فكك احبك طوخ",m:"ا ح ب ك ط و خ"},
